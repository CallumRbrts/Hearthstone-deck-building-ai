...
%----------------------------------------------------------------------------------------
\begin{abstract}
Artificial Intelligence is an exponentially expanding sector touching many departments and found in many forms. Hearthstone is one such department that AI have touched, it has two different aspects AI can touch on, playing the game and building the decks to play. The deck building site has been researched in the form of basic Neural Networks and Genetic Algorithms. These techniques are used often, newer papers using them tend to have slight differences to test performance, however overall the use of these techniques is over-saturated and thus is difficult to research.  \\
\\
This project aims to explore a recently found technique that has been untouched in the deck building sector. This technique is a Generative Adversarial Network, used previously in image generation will in this paper be adapted to produce decks that resemble user created ones in hopes to help develop and evolve the \textit{meta} of Hearthstone. Initial findings found the model to be successful in the production of a deck achieving over 60\% win rate for some iterations, however, data had to be collected manually for the training process. The results produced interesting yet inconsistent results when it came to the testing of the playability of a deck. 

\end{abstract}

\section*{Acknowledgements}
I would like to thank, in particular, my project supervisor Mark Bartlett, whose wisdom proved invaluable to the completion of this project. These thanks extend to the rest of the RGU staff who went beyond expectations in their support during these tough times.
...
\section{Overview}
Bringing Artificial Intelligence to Hearthstone: Heroes of Warcraft is not a new concept, it has been subject to many studies in the field, where it is playing the game, suggesting moves to the player or building decks. Focusing on the field of deck building, it is a concept that exists across multiple trading card games such as Magic: The Gathering, Pokémon, Yu-Gi-Oh! and of course Hearthstone. Papers have studied these games more or less, but all seem to gravitate around similar deck building techniques mostly Genetic Algorithms and Artificial Neural Networks, details of which will be expanded later on. These techniques seemingly been saturated, newer studies apply small scale changes to already defined methods \cite{Back1996}. Exploration of different techniques has been touched on, but they seem to be outliers and of limited number \cite{Ward2020}, experimenting with other algorithms could show better, more  interesting results instead of assuming that one saturated method is the best because of its popular usage. This project will explore the usages of a Generative Adversarial Network (GAN) to develop another method of deck creation. A GAN is a technique that uses a model to create something, could be an image, vector or anything, which is then given to another model to determine is that created something is real or fake. If the first model is able to trick the second model then the generated something is ready to show, the model tweaks itself as the training process goes on to improve results.
\section{Motivation}
...
This section showcases the implementation steps of the project in three distinct sections: Card Data, Deck Data and Generative Adversarial Network. Each section will chronicle their respective development process, whilst discussing potential alternatives and limitations. The goal of this implementation is to initially gather data needed to create the datasets for the GAN, then to convert the datasets into a readable form for the GAN to train. The first section will describe how the data of each card was gathered, what technique was used and improvements entailed. Then with the card data, the second section will cover the collection and creation of the datasets using the card data. Finally the last section will cover the implementation of the GAN itself, which uses both the Card and Deck data to train.
\section{Card Data}
...
Since the file type for the datasets has changes from \texttt{CSV} to JSON, it means that the file loading needs to be modified to work with the updated datasets. Luckily it is straightforward to load JSON data, with the \texttt{json} library, simply load the file (as seen in figure 4.12), there is also a check to only register decks of legal length.
...
After the deck links are collected, another function called \nolinkurl{cards_from_url} is called, this take the URLs as an argument and returns the deck lists in their default format (figure 4.16). It navigates thorough the page into the html table containing the list of cards and gets the names. Finding the correct tables and containers was found through trial and error, but is consistent throughout every page on the site.
...
The changes to \nolinkurl{utils_fireplace.py} are more substantial, the \nolinkurl{play_full_game()} function is changed slightly to pass variables to the \nolinkurl{setup_game()} function (figure 5.2). The rest does not need to be changed because the scope of our testing is focused on deck building rather than improving the AI playing the decks.
...
Before changing the \nolinkurl{setup_game()} function it was coded statically, only a single class was chosen per player. Using the arguments sent by the main, the code was adapted to make it dynamic (figure 5.3), however a \nolinkurl{class_converter()} function was written to convert the string inputs of the two players classes to an object so that the simulator can register them as that player (figure 5.4)
...
This section shows the similarities between a real and a generated deck, in this case a type split, meaning what percentage of a type of card is present in the deck. A successful test would show similar percentages in the two graphs.
...
This test compares the percentage of neutral cards in real and fake decks, and in all cards overall. A successful test would show a generated deck with a similar split as a real deck, whilst being majorly different to the overall cards neutral percentage.
\subsection{Test 1 - Hunter Deck}
\begin{figure}[H]
\centering
...
This test covers the similarity of the mana curves of a real and a generated deck. A successful test would display patterns between the two decks.
\subsection{Test 1 - Hunter Deck}
\begin{figure}[H]
\centering
\begin{subfigure}{.5\textwidth}
...
This test showcases the overall win rates of each generated deck, and against each class. The  deck with the best overall win rate is the one shown in detail and the one the user will most likely select. The decks are tested over 100 matches across 10 classes through the simulator. The simulator is given a deck from the dataset on each repetition, which is used to test the generated deck against.
\subsection{Test 1 - Hunter Deck}
\begin{figure}[H]
\centering
\begin{subfigure}{.5\textwidth}
...
\\
The technique overall does work, it generates playable decks, however the GAN seems to favour some classes over others, possibly due to the play style or the way the decks are built. This technique is more inconsistent than a Genetic Algorithm and Artificial Neural Networks, but it does generate decks with good win rates, further testing with an improved simulator would be needed to clarify this. 
\subsection{Findings}
...
There are several improvements that could be addressed in future work. An improved simulator AI, could help determine the strengths and weaknesses of the algorithm better. The use of a Monty Carlo algorithm would prove useful for such an endeavour. Further refining of the GAN could also be made to improve the outputted deck, an improved dataset which contained cleaner data would also help to achieve this. Improving the GANs consistency over all the classes would be ideal, as of now the GAN favour some classes over others, in the number of decks created, how well they perform.
...
\section{Appendix I - Project Proposal}
\includepdf[page=-]{pp}
\section{Appendix J - Ethics Form}
\includepdf[page=-]{ef}
\section{Appendix K - Poster}
\includepdf[page=-]{hs-ai_poster}
\section{Appendix L - Source Code}
The source code for this program is hosted on GitHub, \url{https://github.com/CallumRbrts/Hearthstone-deck-building-ai}. A copy of this code can also be found within  the  ‘Supplementary Materials Dropbox’  dropbox  on \url{http://campusmoodle.rgu.ac.uk/}. The concerned 'Source Code' files are: \nolinkurl{HS-AI-Demo.ipynb}, \nolinkurl{Card-API.ipynb} and \nolinkurl{Web-Scraper.ipynb}, other Python notebook files are supplementary older versions of the project that do not represent the project in it's final form. Files generated from code such as datasets and images for evaluation can also be found on the Github.
%----------------------------------------------------------------------------------------
